...
function initCanvas () {
    ...
    //testing for dynamic size of shapes
    ghostcanvas = document.createElement('canvas');
    ghostcanvas.zIndex = "200";
...
function handleMoveMessage (coordsString) { //IMPORTANT, PASS CLIENT ID HERE SO WE KNOW WHICH COLOR TO DRAW WITH
    // Parse the specified (x, y) coordinate
    var coords = coordsString.split(",");
    var position = {x:parseInt(coords[0]), y:parseInt(coords[1])};
    userCurrentPosition = {x:position.x, y:position.y};
...
function handlePathMessage (pathString) { //IMPORTANT, PASS CLIENT ID HERE SO WE KNOW WHICH COLOR TO DRAW WITH
    ...
    for (var i = 0; i < path.length; i+=2) {
        ...
        drawLine(defaultLineColor, 
                defaultLineThickness, 
                userCurrentPosition.x, 
                userCurrentPosition.y,
                position.x, 
                position.y);

        userCurrentPosition.x = position.x;
        userCurrentPosition.y = position.y;
        //userCurrentPostion = {x : position.x,y : position.y};	
        //addDrawingCommand(fromClientID, DrawingCommands.LINE_TO, position);
    }
}

function handleRecMessage (pathString) { //IMPORTANT, PASS CLIENT ID HERE SO WE KNOW WHICH COLOR TO DRAW WITH
    // Parse the specified list of points
    var path = pathString.split(",");
...
function broadcastPath () {
    ...
    var contents;
    contents = { "command" : DrawingCommands.LINE_TO, "path" : bufferedPath.join()}
    //contents = "MOVE:" + x + "," + y;
    console.log("inside broadcast PATH");

    for(var i = 0; i<Signaling.Peer.length; i+=1) 
    {
...
function broadcastMove (x, y) {
    ...
    /*msgManager.sendUPC(UPC.SEND_MESSAGE_TO_ROOMS, 
      ...
      x + "," + y);*/
    var contents;
    contents = { "command" : DrawingCommands.MOVE_TO, "path" : x + "," + y}
    //contents = "MOVE:" + x + "," + y;
...
function broadcastRec () {
    ...
    console.log("should send rec record to all participants here" + recRecord.join());

    var contents;
    contents = { "command" : DrawingCommands.REC, "path" : recRecord.join()}
    console.log("inside broadcast PATH");
    ...
    if (isPenDown) {
        ...
        switch (localShape){
            case DrawingShape.line:
                ...
                drawLine(localLineColor, localLineThickness, 
            ...
            case DrawingShape.rec:
                if ((new Date().getTime() - lastBufferTime) > 20) {
                    ...
                    recRecord.y2 = y;
                    lastBufferTime = new Date().getTime();
                    drawRect(gcontext,localLineColor, 
                            localPen.x, localPen.y, x, y);
                }
                break;
        }
...
function penUp () {
    switch (localShape){
        case DrawingShape.rec:
            clearRect(gcontext,recRecord.x1,recRecord.y1, 
                    recRecord.x2,recRecord.y2);
            drawRect(context, localLineColor, recRecord.x1,recRecord.y1, 
                    recRecord.x2,recRecord.y2);
            recRecord = {};
            break;
    }
